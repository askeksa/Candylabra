...
float4 p3(float4 tc : TEXCOORD0) : COLOR0 
{
	...
	{
		u = lp(p+i*dir);
		a += cau(u)*sh(u)*f*b*l.xyz*(2+dot(dir,normalize(s)));
		a *= pow(c, f);
		i += f;
	}
	return float4(a,0);
...
void v4(float4 pi : POSITION, out float4 po : POSITION, out float4 p : TEXCOORD0) {
	p = hash(mul(m,pi).xyz);
	pi.x=p.w;
	//p.xyz+=(sin(p.yzx*14+t)+sin(p.zxy*9+t))*.06; 
	//p.y+=t*.1;
	po = p = mul(vp,  float4( fmod(p.xyz*10-e+5+1000,10) -5 + e ,1));
	p.xyz/=p.w;
	p.w = frac(pi.x);
}
...
{
	//return float4(tex2D(dt,tc.xy*float2(.5,-.5)+.5).x,0,0,1);
	if (tex2D(dt,tc.xy*float2(.5,-.5)+.5).x < tc.z) discard;
	//return float4(1,.9,.8,.4);
	//return float4(tc.z,0,0,1);
	return c*tc.w*(1-tc.z*3);
}
...
float4 p5(float4 tc : TEXCOORD0) : COLOR0 
{
	...
	float w,ws=0;
	float4 a=0,cc,p = mul(ivp,float4(tc.xy,tex2D(dt,u).x,1));
	p/=p.w; 
	...
	p = tex2D(ct,u);
	return (sqrt(p*pow(c,p.w*16) + a) - dot(tc.xy,tc.xy)*.2)*f;
}   
